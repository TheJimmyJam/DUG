              ...
              <Card className="transition-shadow group-hover:shadow-md h-full">
                <CardContent className="pt-5 pb-5 h-full">
                  <div className="flex items-start gap-4 h-full">
                    ...
                    {(() => {
                      const ri = computeReputationIndex(p);
                      return (
                        <div className="text-right shrink-0">
                          <div className={`text-sm font-bold ${ri.hasSignal ? "text-[var(--color-fg)]" : "text-[var(--color-muted)]"}`}>
                            {ri.hasSignal ? ri.score : "—"}
                            <span className="ml-1 text-xs font-normal text-[var(--color-muted)]">RI</span>
                          </div>
                          {ri.hasSignal && (
                            <div className={`text-xs font-semibold ${RI_TIER_COLORS[ri.tier]}`}>{ri.tier}</div>
                          )}
                          {!ri.hasSignal && (
                            <div className="text-xs text-[var(--color-muted)]">No signal yet</div>
                          )}
                        </div>
                      );
                    })()}
