...
// Main component
// ===========================================================================
export function AdminPanel({ initialUsers, initialJobs, initialPricingConfig }: AdminPanelProps) {
  const [tab, setTab] = useState<"users" | "jobs" | "demo" | "pricing">("users");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b">
        {(["users", "jobs", "pricing", "demo"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
      ...
      {/* Tab content */}
      {tab === "users" && (
        <UsersTab users={users} onRefresh={refreshUsers} refreshing={pending} />
      )}
      {tab === "jobs" && (
        <JobsTab jobs={jobs} onRefresh={refreshJobs} refreshing={pending} />
      )}
      {tab === "pricing" && (
        <PricingTab initialConfig={initialPricingConfig} />
      )}
      {tab === "demo" && (
...
// PRICING TAB
// ===========================================================================
function PricingTab({ initialConfig }: { initialConfig: PricingConfig }) {
  const [config, setConfig] = useState<PricingConfig>(JSON.parse(JSON.stringify(initialConfig)));
  const [saving, startSave] = useTransition();
  const [msg, setMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  function setFee(tier: string, bps: number) {
    setConfig((c) => ({ ...c, platformFees: { ...c.platformFees, [tier]: bps } }));
  }

  function setDifficultyRange(level: string, side: "flat" | "hourly", bound: "min" | "max", val: number) {
    setConfig((c) => ({
      ...c,
      byDifficulty: {
        ...c.byDifficulty,
        [level]: {
          ...c.byDifficulty[level],
          [side]: { ...c.byDifficulty[level][side], [bound]: val },
        },
      },
    }));
  }

  function setOverride(jobType: string, bound: "min" | "max", val: number) {
    setConfig((c) => ({
      ...c,
      jobTypeOverrides: {
        ...c.jobTypeOverrides,
        [jobType]: { ...c.jobTypeOverrides[jobType], [bound]: val },
      },
    }));
  }

  function removeOverride(jobType: string) {
    setConfig((c) => {
      const next = { ...c.jobTypeOverrides };
      delete next[jobType];
      return { ...c, jobTypeOverrides: next };
    });
  }

  function addOverride() {
    const type = prompt("Job type key (e.g. program_design):");
    if (!type?.trim()) return;
    setConfig((c) => ({
      ...c,
      jobTypeOverrides: { ...c.jobTypeOverrides, [type.trim()]: { min: 0, max: 0 } },
    }));
  }

  function setBounty(field: keyof PricingConfig["bounty"], val: number) {
    setConfig((c) => ({ ...c, bounty: { ...c.bounty, [field]: val } }));
  }

  function save() {
    setMsg(null);
    startSave(async () => {
      const res = await savePricingConfigAction(config);
      setMsg(res.ok ? { type: "ok", text: "Saved." } : { type: "err", text: res.error });
    });
  }

  const feeLabels: Record<string, string> = {
    consumer: "Consumer (individuals)", professional: "Professional (brokers/agents/risk mgrs)",
    enterprise: "Enterprise (carriers/MGAs/reinsurers)", strategic: "Strategic (annual license)",
  };

  return (
    <div className="space-y-8 max-w-2xl">
      <p className="text-sm text-[var(--color-muted)]">
        All ranges are advisory hints shown to requesters — never enforced as minimums or maximums. Fee rates take effect on new engagements only.
      </p>

      {/* Platform fees */}
      <section>
        <h2 className="text-lg font-semibold mb-3">Platform fee rates</h2>
        <div className="space-y-3">
          {Object.entries(config.platformFees).map(([tier, bps]) => (
            <div key={tier} className="flex items-center gap-4">
              <div className="w-64 text-sm text-[var(--color-muted)]">{feeLabels[tier] ?? tier}</div>
              <div className="flex items-center gap-2">
                <Input
                  type="number" min={0} max={10000} step={50}
                  className="w-24"
                  value={bps / 100}
                  onChange={(e) => setFee(tier, Math.round(parseFloat(e.target.value) * 100) || 0)}
                />
      ...
      <section>
        <h2 className="text-lg font-semibold mb-1">Suggested ranges by complexity</h2>
        <p className="text-xs text-[var(--color-muted)] mb-4">Shown as hints in the post-engagement form. All values in USD.</p>
        <div className="space-y-4">
          {Object.entries(config.byDifficulty).sort(([a], [b]) => Number(a) - Number(b)).map(([level, row]) => (
            <div key={level} className="rounded-lg border p-4 space-y-3">
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold">Complexity {level}</span>
                <input
                  className="flex-1 rounded border border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1 text-sm"
                  value={row.label}
                  onChange={(e) => setConfig((c) => ({
                    ...c,
                    byDifficulty: { ...c.byDifficulty, [level]: { ...row, label: e.target.value } },
                  }))}
                  placeholder="Label (e.g. Complex)"
                />
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                {(["flat", "hourly"] as const).map((side) => (
                  <div key={side}>
                    <div className="mb-1 text-xs font-medium uppercase tracking-wide text-[var(--color-muted)]">
                      {side === "flat" ? "Flat fee ($)" : "Hourly ($/hr)"}
                    </div>
                    <div className="flex items-center gap-2">
                      <Input type="number" min={0} className="w-24" placeholder="Min"
                        value={row[side].min}
                        onChange={(e) => setDifficultyRange(level, side, "min", parseInt(e.target.value) || 0)} />
                      <span className="text-[var(--color-muted)]">–</span>
                      <Input type="number" min={0} className="w-24" placeholder="Max"
                        value={row[side].max}
                        onChange={(e) => setDifficultyRange(level, side, "max", parseInt(e.target.value) || 0)} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* Job type overrides */}
      <section>
        <h2 className="text-lg font-semibold mb-1">Job type overrides (flat fee)</h2>
        <p className="text-xs text-[var(--color-muted)] mb-4">These override the complexity-based hint for specific engagement types.</p>
        <div className="space-y-2">
          {Object.entries(config.jobTypeOverrides).map(([jobType, range]) => (
            <div key={jobType} className="flex items-center gap-3">
              <div className="w-44 text-sm font-mono text-[var(--color-muted)]">{jobType}</div>
              <Input type="number" min={0} className="w-24" placeholder="Min"
                value={range.min}
                onChange={(e) => setOverride(jobType, "min", parseInt(e.target.value) || 0)} />
              <span className="text-[var(--color-muted)]">–</span>
              <Input type="number" min={0} className="w-24" placeholder="Max"
                value={range.max}
                onChange={(e) => setOverride(jobType, "max", parseInt(e.target.value) || 0)} />
              <button type="button" onClick={() => removeOverride(jobType)}
                className="text-[var(--color-muted)] hover:text-[var(--color-danger)] text-lg leading-none">×</button>
            </div>
          ))}
        </div>
        <button type="button" onClick={addOverride}
          className="mt-3 text-xs text-[var(--color-primary)] hover:underline">
          + Add job type override
        </button>
      </section>

      {/* Bounty defaults */}
      <section>
        <h2 className="text-lg font-semibold mb-3">Bounty defaults (per-find)</h2>
        <div className="grid grid-cols-3 gap-4">
          {(["defaultPerFind", "suggestedMin", "suggestedMax"] as const).map((field) => (
            <div key={field}>
              <div className="mb-1 text-xs font-medium text-[var(--color-muted)]">
                {field === "defaultPerFind" ? "Default per find ($)" : field === "suggestedMin" ? "Suggested min ($)" : "Suggested max ($)"}
              </div>
              <Input type="number" min={0}
                value={config.bounty[field]}
                onChange={(e) => setBounty(field, parseInt(e.target.value) || 0)} />
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="flex items-center gap-4 border-t pt-6">
        ...
        {msg && (
          <span className={`text-sm ${msg.type === "ok" ? "text-[var(--color-success)]" : "text-[var(--color-danger)]"}`}>
            {msg.text}
          </span>
        )}
      </div>
