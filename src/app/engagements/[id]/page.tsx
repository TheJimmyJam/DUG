...
export default async function JobDetailPage({ params }: PageProps) {
  ...
  const [{ data: job }, { data: { user } }] = await Promise.all([
    supabase
      .from("jobs")
      .select(
        `*, poster:profiles!jobs_poster_id_fkey(handle, display_name, is_verified), claimer:profiles!jobs_claimed_by_fkey(handle, display_name, rating, rating_count, completed_job_count, years_experience, is_verified)`,
      )
      .eq("id", id)
      .maybeSingle(),
              ...
              <CardContent className="pt-6">
                ...
                {isClaimer && (
                  <div className="space-y-3">
                    ...
                    <Link
                      href={`/engagements/${job.id}/submit`}
                      className="block"
                    >
                      <Button variant="primary" className="w-full">
                ...
                {isPoster && job.claimer && (
                  <div className="space-y-2">
                    ...
                    {(() => {
                      const ri = computeReputationIndex(job.claimer);
                      return ri.hasSignal ? (
                        <div className="text-xs text-[var(--color-muted)]">
                          RI{" "}
                          <span className="font-semibold text-[var(--color-fg)]">{ri.score}</span>
                          {" · "}
                          <span className={RI_TIER_COLORS[ri.tier]}>{ri.tier}</span>
                        </div>
                      ) : null;
                    })()}
                  </div>
                ...
                {isPoster && job.status === "submitted" && (
                  <Link
                    href={`/engagements/${job.id}/review`}
